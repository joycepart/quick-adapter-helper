...
public class PtrFrameLayout extends ViewGroup {
    ...
    public boolean isFixtoHeaderHeight() {
        return fixtoHeaderHeight;
    }

    public void setFixtoHeaderHeight(boolean fixtoHeaderHeight) {
        this.fixtoHeaderHeight = fixtoHeaderHeight;
    }

    boolean fixtoHeaderHeight = false;
    ...
    private void movePos(float deltaY) {
        ...
        if (fixtoHeaderHeight) {
            to = (to > mPtrIndicator.getOffsetToRefresh() ? mPtrIndicator.getOffsetToRefresh() : to);
        }
    ...
    private void updatePos(int change) {
        ...
        if (mPtrUIHandlerHolder.hasHandler()) {
            ...
            if (fixtoHeaderHeight && mPtrIndicator.getCurrentPosY() > mPtrIndicator.getOffsetToRefresh() && mPtrIndicator.getLastPosY() <= mPtrIndicator.getOffsetToRefresh()) {
                if (isUnderTouch && mStatus == PtrFrameLayout.PTR_STATUS_PREPARE) {
                    mPtrIndicator.setFix(true);
                }
            }
        }
